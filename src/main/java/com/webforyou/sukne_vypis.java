...
@WebServlet(name = "sukne_vypis")
public class sukne_vypis extends HttpServlet {
    Connection con = null;
    ...
    private void vypisVsetko(PrintWriter out) {
        try {
            ...
            ResultSet rs = stmt.executeQuery("SELECT * FROM inzerat INNER JOIN kategorie ON inzerat.idKategorie = kategorie.idKategorie WHERE Nazov_kategorie = 'Sukne' ORDER BY idProduktu DESC");
...
//                out.println("<button class='card-text text-left' value='vymazat' name='operacia' style='background-color:transparent; width:30px; color: black; border:0px; outline:none;'>❌</button>");
                ...
                out.println("<p class='card-text'>" + rs.getString("psc") + "</p>");
