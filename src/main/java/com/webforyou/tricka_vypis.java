...
@WebServlet(name = "tricka_vypis")
public class tricka_vypis extends HttpServlet {
    Connection con = null;
    ...
    private void vypisVsetko(PrintWriter out) {
        try {
            ...
            ResultSet rs = stmt.executeQuery("SELECT * FROM inzerat INNER JOIN kategorie ON inzerat.idKategorie = kategorie.idKategorie WHERE Nazov_kategorie = 'Tricka' ORDER BY idProduktu DESC");
...
//                out.println("<button class='card-text text-left' value='vymazat' name='operacia' style='background-color:transparent; width:30px; color: black; border:0px; outline:none;'>❌</button>");
                ...
                out.println("<p class='card-text'>psc: " + rs.getString("inzerat.psc") + "</p>");
